...
One can get the Nth character, or letter from a string by writing 
`"<string-liateral>"[N]` The returned value will be a string containing
only one character (for example, "b"). The first character has position 
0, which causes the last one ot be found at position 
`<string-object-ref>.length-1`. In other words, a two-character string
...
Take care to make all the bindings used in the function local to the function by 
properly declaring them with the let or const keyword. -->
